package generic.db.gateway.service;

import generic.db.gateway.config.EndpointConfig;
import generic.db.gateway.config.MyBatisFactory;
    ...
    public CompletableFuture<SqlExecutionResult> execute(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                LOG.error("SQL execution failed: {} - {}", endpointConfig.sqlId(), e.getMessage());
                return SqlExecutionResult.forError(endpointConfig.sqlType(), e.getMessage());
            }
        }, virtualExecutor);
    }
